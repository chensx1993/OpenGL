...
int main()
{
    ...
    // ------------------------------------
    Shader ourShader("GLSL/6.1.cubemaps.vs", "GLSL/6.1.cubemaps.fs");
    Shader skyboxShader("GLSL/6.1.skybox.vs", "GLSL/6.1.skybox.fs");
...
unsigned int loadCubemap(vector<std::string> faces)
{
    ...
    for (unsigned int i = 0; i < faces.size(); i ++) {
        unsigned char *data = stbi_load(faces[i].c_str(), &width, &height, &nrChanels, 0);
        if (data) {
