...
Shader::Shader(const GLchar* vCodeString, const GLchar* fCodeString, GLboolean isCode)
{
    if (isCode == GL_FALSE) {
        Shader(vCodeString, fCodeString);
    }else {
    
        GLuint vShaderId = createShader(vCodeString, ShaderTypeVertex);
        GLuint fShaderId = createShader(fCodeString, ShaderTypeFragment);
